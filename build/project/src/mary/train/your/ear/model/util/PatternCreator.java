...
public class PatternCreator {
	 ...
	 */
	public static LinkedList<Pattern> createPatterns(LinkedList<Interval> structures) throws PatternCreationException {
		LinkedList<Pattern> patterns = new LinkedList<>();
	...
	private static String createPatternForInterval(Interval structure, String connector, double lowestPitch) {
		...
		System.out.println("first: " + Note.getFrequencyForNote((int) (double) list.get(0)) + " "
				+ Note.getFrequencyForNote((int) (double) list.get(1)));
		return sb.toString();
	}
	...
	public static LinkedList<Pattern> createPatternsForChords(LinkedList<CustomChord> structures) {
		...
		structures.forEach(struct -> {
			...
			p.setTempo(GeneralTaskSettings.getSettings().getTempo());
			patterns.add(p);
			System.out.println(struct + StringConstants.MEL_CONNECTOR + p.toString());
		});
		return patterns;
	}
	...
	private static void createPattern(IntervalComparatorElement element) {
		...
		System.out.println(element.getFirstCompareToSecond() + " " + baseOfFirst + " " + baseOfSecond);
		...
		System.out.println(
				element.getFirstCompareToSecond() + " " + element.getPatterns()[0] + " " + element.getPatterns()[1]);
	}
	...
	private static Pattern createSecondPattern(IntervalComparatorElement element, String connector,
		...
		System.out.println("higherFre: " + higherFreq + " lowerFreq" + Note.getFrequencyForNote((int) baseOfSecond));
	...
	/*
	 * public static void main(String[] args) { ChordStrategy chordPlayer = new
	 * ChordStrategy(); ChordsResponse response = (ChordsResponse)
	 * chordPlayer.play(); System.out.println(response.getAnswer() + "\n" +
